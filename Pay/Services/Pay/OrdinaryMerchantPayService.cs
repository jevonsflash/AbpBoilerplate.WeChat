...
using WeChat.Pay.Services;

namespace WeChat.Pay.Services.Pay
{
    /// <summary>
    /// 集成了普通商户的微信支付 API 接口。
    /// </summary>
    public class OrdinaryMerchantPayService : WeChatPayService
    {
        ...
        public OrdinaryMerchantPayService(IPayConfiguration options) : base(options)
        {
        ...
        {
            var billCreateIp = "192.168.1.2";

            return await UnifiedOrderAsync(appId, mchId, null, body, null, attach,
                outTradeNo, null, totalFee, billCreateIp, null, null,
        ...
        /// <param name="mchId">微信支付分配的商户号。</param>
        /// <param name="weChatOrderNo">微信订单号。</param>
        /// <param name="weChatRefundNo">微信退款单号</param>
        /// <param name="refundNo">商户退款单号</param>
        /// <param name="orderNo">商户订单号，该订单号是商户系统内部生成的唯一编号。</param>
        /// <returns>请求的结果，会被转换为 <see cref="XmlDocument"/> 实例并返回。</returns>
        /// <exception cref="ArgumentException">微信订单号,商户订单号,微信退款单号，商户退款单号都为 null 时，会抛出本异常。</exception>
        public async Task<XmlDocument> RefundQueryAsync(string appId, string mchId, string weChatOrderNo = null, string orderNo = null, string weChatRefundNo = null, string refundNo = null)
        {
            if (string.IsNullOrEmpty(weChatOrderNo) && string.IsNullOrEmpty(orderNo) && string.IsNullOrEmpty(weChatRefundNo) && string.IsNullOrEmpty(refundNo))
            {
                throw new ArgumentException("微信订单号,商户订单号,微信退款单号，商户退款单号，必须4选1传递一个有效参数。");
            }
            var request = new WeChatPayParameters();
            request.AddParameter("appid", appId);
            request.AddParameter("mch_id", mchId);
            ...
            request.AddParameter("transaction_id", weChatOrderNo);
            request.AddParameter("out_trade_no", orderNo);
            request.AddParameter("refund_id", weChatRefundNo);
            request.AddParameter("out_refund_no", refundNo);
